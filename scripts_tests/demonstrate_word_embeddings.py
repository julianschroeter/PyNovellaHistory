...
my_language_model_de = "de_core_news_lg" #"de_dep_news_trf"
...
list_of_keywords=["Loyalität", "Verrat", "Virus"]
...
all_output_words.extend(["König-Mann+Frau"])
print(all_output_words)


#colors_list = ["red" if word in compl_words else "blue" if word in list_of_keywords else "green" for word in all_output_words]
colors_list = ["blue" if word in list_of_keywords else ("red" if word in "König-Mann+Frau" else "green") for word in all_output_words]
...
plt.title("Word embedding und latente Semantik")
plt.show()
