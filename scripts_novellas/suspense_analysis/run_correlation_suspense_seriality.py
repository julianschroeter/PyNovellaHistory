...
df_serial = df[df.isin({"seriell": ["TRUE", "True", "vermutlich"]}).any(1)]
df_nonserial = df[df.isin({"seriell": ["False", "FALSE"]}).any(1)]
...
for x_variable in x_variables:
    ...
    plt.plot(df_serial.loc[:, x_variable], y_pred, color="black", linewidth=3)
    ...
    plt.plot(df_nonserial.loc[:, x_variable], y_pred, color="grey", linewidth=3)
