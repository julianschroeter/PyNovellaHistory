...
whole_df = whole_df[whole_df.isin({"genre": labels_list}).any(1)]
...
df_canon3 = df[df["Kanon_Status"]== 3]
...
canon_mpatches_list = []
for canon, color in {3:"black", 0:"grey"}.items():
...
for x_variable in x_variables:
    ...
    plt.plot(df_canon0.loc[:, x_variable], y_pred, color="grey", linewidth=3)

    plt.scatter(df_canon3.loc[:, x_variable], df_canon3.loc[:, y_variable], color="black")  # authors_colors_list
    regr = LinearRegression()
    ...
    plt.plot(df_canon3.loc[:, x_variable], y_pred, color="black", linewidth=3)
