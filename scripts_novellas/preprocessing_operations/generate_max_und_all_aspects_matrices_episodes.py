...
sent_fear_path = os.path.join(global_corpus_representation_directory(system), "DocSentFearMatrix_novellas_episodes.csv")

spuk_path = os.path.join(global_corpus_representation_directory(system), "Spuk_DocThemesMatrix_novellas_episodes.csv")
hardseed_path = os.path.join(global_corpus_representation_directory(system), "DocHardseedsMatrix_episodes.csv")
...
danger_df = df.drop(columns=["Erotik", "Liebe", "SentLexFear", "embedding_Angstempfinden","UnbekannteEindr", "Angstempfinden", "großzügig", "verwerflich", "Sturm", "Feuer", "Dämon", "Hardseeds"])
...
endanger_char_df = pd.read_csv(os.path.join(global_corpus_representation_directory(system), "DocNamesCounterMatrix_novellas_episodes.csv"), index_col=0)
...
print(df)
df.to_csv(os.path.join(local_temp_directory(system), "MaxDangerFearCharactersHardseeds_novellas_episodes_scaled.csv"))

print(danger_df.max_danger_typ.values)

danger_df.to_csv(os.path.join(local_temp_directory(system), "AllChunksDangerFearCharactersHardseeds_novellas_episodes_scaled.csv"))
