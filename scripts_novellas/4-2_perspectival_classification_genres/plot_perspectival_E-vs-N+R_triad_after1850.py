...
infile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_E-R-after1850_n100.txt")
E_R_data_matrix_filepath = os.path.join(local_temp_directory(system), "av_pred_probabs_E-R-after1850_n100.csv")
...
infile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_N-E-after1850_n100.txt")
with open(infile_output, "r") as f:
    lines = f.readlines()
optimum_x = str(lines[0]).split(": ")[1]
optimum_x = optimum_x.replace("\n", "")
...
plt.title("1850–1950: Distanz der Erzählungen zu Novelle und Roman")
