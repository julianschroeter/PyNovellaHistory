...
probabs_dict = {}
acc_scores, acc_scores2, N_f1_scores, E_f1_scores = [], [], [],[]
optima_x, optima_y = [],[]
...
for filename in os.listdir(global_corpus_raw_dtm_directory(system)):
    if filename == "red-to-2500mfw_scaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv": #"raw_dtm_l1_lemmatized_use_idf_False2500mfw.csv": # alternative: "no-names_RFECV_red-to-515_LRM-R-N-E-0E-XEscaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv"
        ...
        for i in range(n):
            ...
            acc_scores2.append(accuracy_score(Y_test, lr_model.predict(X_test)))
...
print("mean of accuracy scores, alternative: ", np.mean(acc_scores2))
...
print(df)
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_features_coefs_n100_N-E_after1850.csv"))
...
plt.savefig("/home/julian/git/PyNovellaHistory/figures/gridsuche_c-at-one_N-E.svg")
plt.show()


outfile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_N-E_n100_after1850.txt")
...
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_pred_probabs_N-E_n100_after1850.csv"))
