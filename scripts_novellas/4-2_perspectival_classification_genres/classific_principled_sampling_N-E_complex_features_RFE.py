...
list_optimal_numbers, selected_features_list = [], []
optima_x, optima_y = [],[]
    ...
    if filename ==  "doc_complex_features_matrix.csv":
        ...
        for i in range(n):
          ...
          #  df = df[df["Jahr_ED"] >= 1850]
            df_all = df.copy()
           # df = df.loc[:, ["rom_setting", "Gattungslabel_ED_normalisiert", "Jahr_ED", "Nachname"]]
            ...
            selector = RFECV(lr_model, step=1, min_features_to_select=2, cv=3)
            ...
            dropped_input_df = df_all.drop(columns=["Gattungslabel_ED_normalisiert", "Nachname", "Jahr_ED"])
            new_df = dropped_input_df.loc[:, supported_features]
            print(new_df)
            print("Optimal number of features : %d" % selector.n_features_)
            list_optimal_numbers.append(selector.n_features_)

            features = new_df.columns.tolist()
            selected_features_list.extend(features)

            print(selector.ranking_)
            print(selector.cv_results_)

            test_predictions = selector.predict(X_test)


            #print("coef with features: ", sorted(coef_features, key=lambda x: x[0]))

            acc_scores.append(accuracy_score(Y_test, selector.predict(X_test)))
            N_f1_scores.append(f1_score(Y_test, selector.predict(X_test), pos_label=1))
            E_f1_scores.append(f1_score(Y_test, selector.predict(X_test), pos_label=0))

            print(classification_report(Y_test, test_predictions))
            predict_probs_inv = [x for sublist in selector.predict_proba(X_test)[:,0:1] for x in  sublist]
            predict_probs = [x for sublist in selector.predict_proba(X_test)[:, 1:2] for x in sublist]
...
print(np.mean(list_optimal_numbers))
print(Counter(selected_features_list))