...
probabs_dict = {}
acc_scores, E_f1_scores, R_f1_scores = [], [],[]
optima_x, optima_y = [],[]
        ...
        for period in periods:
            if period == "1850-1950":
                ...
                for i in range(n):
                    ...
                    E_f1_scores.append(f1_score(Y_test, lr_model.predict(X_test), pos_label=1))
                    R_f1_scores.append(f1_score(Y_test, lr_model.predict(X_test), pos_label=0))
                ...
                results_dict[period] = [np.mean(np.asarray(acc_scores)), np.mean(np.asarray(E_f1_scores)), np.mean(np.asarray(R_f1_scores))]
