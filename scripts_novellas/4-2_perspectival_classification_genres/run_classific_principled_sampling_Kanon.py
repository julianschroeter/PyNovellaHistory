...
class_cat = "Kanon_Status" # "in_Deutscher_Novellenschatz"
...
for filename in os.listdir(global_corpus_raw_dtm_directory(system)):
    if filename == "red-to-2500mfw_scaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv": # "raw_dtm_l1_lemmatized_use_idf_False2500mfw.csv": # alternative: "no-names_RFECV_red-to-515_LRM-R-N-E-0E-XEscaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv"
        ...
        for i in range(n):
            ...
            replace_dict = {class_cat: {3: "Kanon-Novelle", 2: "sonstige MLP", 1: "sonstige MLP", 0: "sonstige MLP"}}

            dtm_obj.data_matrix_df = full_genre_labels(dtm_obj.data_matrix_df, replace_dict=replace_dict)


            df = dtm_obj.data_matrix_df
            df_0 = df[df[class_cat] == "Kanon-Novelle"]
            ...
            subs_dict = {"Kanon-Novelle": 1, "sonstige MLP": 0}
...
print(df)
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_features_coefs_n10_Kanon.csv"))
...
obj = obj.add_metadata([class_cat, name_cat, "Titel"])
df = obj.data_matrix_df
print(df)
means = df["mean"].values.tolist()
predict_probs_inv = [1 - value for value in means]
predict_probs = means
labels = df[class_cat].values.tolist()

subs_dict = {"Kanon-Novelle": "orange", "sonstige MLP": "cyan"}
...
outfile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_Kanon_n10.txt")
...
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_pred_probabs_Kanon_n10.csv"))

#plot_prototype_concepts(predict_probs_inv, genre_c_labels, threshold=optimum_x, annotation=annotation)

print(thresholds_scores_plot)
print("Improvement: ", optimum_Y / np.mean(acc_scores))
