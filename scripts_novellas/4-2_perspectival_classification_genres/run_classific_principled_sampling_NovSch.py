...
for filename in os.listdir(global_corpus_raw_dtm_directory(system)):
    if filename == "red-to-2500mfw_scaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv": # "raw_dtm_l1_lemmatized_use_idf_False2500mfw.csv": # alternative: "no-names_RFECV_red-to-515_LRM-R-N-E-0E-XEscaled_raw_dtm_lemmatized_l1__use_idf_False6000mfw.csv"
        ...
        for i in range(n):
            ...
            replace_dict = {class_cat: {True: "im Novellenschatz", False: "sonstige MLP", 0: "sonstige MLP"}}

            dtm_obj.data_matrix_df = full_genre_labels(dtm_obj.data_matrix_df, replace_dict=replace_dict)


            df = dtm_obj.data_matrix_df
            df_0 = df[df[class_cat] == "im Novellenschatz"]
            ...
            subs_dict = {"im Novellenschatz": 1, "sonstige MLP": 0}
...
print(df)
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_features_coefs_n10_NovSch.csv"))
...
outfile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_NovSch_n10.txt")
...
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_pred_probabs_NovSch_n10.csv"))
