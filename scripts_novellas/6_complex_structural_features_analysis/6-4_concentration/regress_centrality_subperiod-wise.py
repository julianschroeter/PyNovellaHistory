...
infile_name = os.path.join(local_temp_directory(system), "conll_based_networkdata-matrix-novellas.csv")
infile_name = os.path.join(global_corpus_representation_directory(system), "Network_Matrix_all.csv")
metadata_filepath = os.path.join(global_corpus_representation_directory(system), "SNA_novellas.csv")
...
matrix_obj = DocFeatureMatrix(data_matrix_filepath=None, data_matrix_df=matrix_obj.data_matrix_df, metadata_csv_filepath= metadata_filepath)
...
matrix_obj.data_matrix_df = years_to_periods(input_df=matrix_obj.data_matrix_df, category_name="Jahr_ED", start_year=1750, end_year=1910, epoch_length=100,
...
periods20a = ["1790-1810", "1810-1830", "1830-1850", "1850-1870", "1870-1890", "1890-1910", "1910-1930"]
...
for period in periods20a:
    ...
    if period != 0:
        ...
        #df = df.fillna(0)
        #df["av_density"] = df.apply(lambda x: (x["density"] + x["Netzwerkdichte"]) / 2, axis=1)
