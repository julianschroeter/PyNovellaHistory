...
sns.lineplot(data=df, x="Jahr_ED", y="dep_var", hue="Gattungslabel_ED_normalisiert",
             palette=["red", "orange", "green"])
plt.title("Gattungen nach Grad an Zentralisierung")
plt.show()

sns.lineplot(data=df, x="Jahr_ED", y="dep_var", hue="Medientyp_ED")
plt.show()
...
with pm.Model() as model:  # model specifications in PyMC3 are wrapped in a with-statement
   ...
   # Define likelihood
   likelihood = Normal("Y", mu=intercept + genre_E_coeff * X["Gattungslabel_ED_normalisiert_Erzählung"] +
                        genre_N_coeff * X["Gattungslabel_ED_normalisiert_Novelle"] +
...
plt.tight_layout()
plt.savefig(os.path.join(local_temp_directory(system), "pymc_traceplot_regress_centrality.png"))
plt.show()

az.plot_posterior(trace)
plt.tight_layout()
plt.savefig(os.path.join(local_temp_directory(system), "pymc_plotposterior_regress_centrality.png"))
plt.show()
...
print(ann_bambi)
print(bambi_summary)
bambi_summary.to_csv(os.path.join(local_temp_directory(system), "bamby_centrality_summary.csv"))