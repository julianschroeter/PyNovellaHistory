...
matrix_obj.data_matrix_df = years_to_periods(matrix_obj.data_matrix_df, category_name="Jahr_ED", start_year=1850, end_year=1970, epoch_length=20, new_periods_column_name="periods")
...
matrix_obj = matrix_obj.reduce_to_categories("periods", ["1850-1870"]) #, "1815-1822" , "1825-1845"
matrix_obj = matrix_obj.reduce_to_categories("medium", ["gartenlaube","daheim"])
...
daheim_N_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Novelle'][df["medium"] == "daheim"]
daheim_E_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Erzählung'][df["medium"] == "daheim"]
daheim_otherE_df = df["token_count"][df.isin({"Gattungslabel_ED_normalisiert": ["kein Label", "andere Label"]}).any(axis=1)][df["medium"] == "daheim"]

daheim_MLP0_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'kein Label'][df["medium"] == "daheim"]
...
grouped_df.unstack().plot(kind='bar', stacked=False,
                                       title=str("Entwicklung der Textlänge: Prosa in Daheim"),
                                        color=["green", "red", "orange", "blue", "yellow"],
                                      ax=axes[1])
left, right = axes[1].set_xlim()
