...
genre_selections = ["all_genres", "N-vs-E-vs-MLP", "N-vs-MLP", "N-vs-E-vs-MLP-vs-R","N-vs-E-vs-MLP-vs-M", "N-vs-E"]
media_selections = ["all_media", "TB-Ant-RS-FB", "TB-Ant-RS-FB-Journal", "TB-Journal-Book", "Buch-vs-Journal"]
...
for triple in triples:
    ...
    if j == "all_genres":
        ...
        replace_dict = {"Gattungslabel_ED_normalisiert": {"N": "N", "E": "MLP", "0E": "MLP",
                                                          "XE": "MLP"}}
        df = full_genre_labels(df, replace_dict=replace_dict)
    elif j == "N-vs-E-vs-MLP-vs-R":
        cat_labels = ["N", "E", "0E", "XE", "R"]
        df = df[df.isin({"Gattungslabel_ED_normalisiert": cat_labels}).any(axis=1)]
        replace_dict = {"Gattungslabel_ED_normalisiert": {"N": "N", "E": "E", "0E": "MLP",
                                                          "XE": "MLP"}}
        df = full_genre_labels(df, replace_dict=replace_dict)
    elif j == "N-vs-E-vs-MLP-vs-M":
    ...
    if k == "all_media":
        ...
        df = full_genre_labels(df, replace_dict=replace_dict)
    elif k == "Buch-vs-Journal":
        # cat_labels = ["Taschenbuch", "Rundschau", "Anthologie", "Familienblatt", "Zeitung", "Zeitschrift", "Buch", "Werke"]
        # df = df[df.isin({"Medientyp_ED": cat_labels}).any(1)]
        replace_dict = {"Medientyp_ED": {"Zeitung": "Journal", "Zeitschrift": "Journal", "Illustrierte": "Journal",
                                         "Rundschau": "Journal", "Familienblatt": "Journal", "Taschenbuch":"Journal",
                                         "Werke": "Buch", "Nachlass": "Buch",
                                         "Zyklus": "Buch", "Sammlung": "Buch"
                                         }}
    ...
    R2 = oos_current_model_r2
    n = df.shape[0]
    p = df.shape[1]
    adj_oos_current_model_r2 = 1 - (1 - R2) * (n - 1) / (n - p - 1)
    train_current_model_r2 = r2_score(y_train, model.predict(X_train))
    ...
    all_model_params.append([i, j, k, oos_current_model_r2, adj_oos_current_model_r2])
from operator import itemgetter
sorted_params = sorted(all_model_params, key = itemgetter(3))

params_df = pd.DataFrame(sorted_params)
print(params_df)
params_df.to_csv(os.path.join(local_temp_directory(system), "params_grid_search_length_media_genre.csv"))
...
ls = sm.OLS(y_train,np.asarray(X_train_Sm)).fit()
