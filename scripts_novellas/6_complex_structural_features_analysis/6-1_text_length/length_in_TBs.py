...
matrix_obj.data_matrix_df = years_to_periods(matrix_obj.data_matrix_df, category_name="Jahr_ED", start_year=1805, end_year=1970, epoch_length=20, new_periods_column_name="periods")
...
aglaja_N_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Novelle'][df["medium"] == "aglaja"]
aglaja_E_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Erzählung'][df["medium"] == "aglaja"]

aglaja_MLP0_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'kein Label'][df["medium"] == "aglaja"]
aglaja_all_MLP_df = df[df.isin({"Gattungslabel_ED_normalisiert": ["Novelle", "Erzählung", "MLP0"]}).any(axis=1)][df["medium"] == "aglaja"]
aglaja_otherE_df = df["token_count"][df.isin({"Gattungslabel_ED_normalisiert": ["kein Label", "andere Label"]}).any(axis=1)][df["medium"] == "aglaja"]

urania_N_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Novelle'][df["medium"] == "urania"]
urania_E_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'Erzählung'][df["medium"] == "urania"]
urania_otherE_df = df["token_count"][df.isin({"Gattungslabel_ED_normalisiert": ["kein Label", "andere Label"]}).any(axis=1)][df["medium"] == "urania"]

urania_MLP0_df = df['token_count'][df['Gattungslabel_ED_normalisiert'] == 'kein Label'][df["medium"] == "urania"]
...
axes[0].boxplot([aglaja_N_df.values.tolist(), aglaja_E_df.values.tolist(), aglaja_otherE_df.values.tolist()], labels=["Novellen", "Erzählungen", "sonst_MLP"])
axes[0].set_title("Aglaja")
...
axes[1].boxplot([urania_N_df.values.tolist(), urania_E_df.values.tolist(), urania_otherE_df],
                labels=["Novellen", "Erzählungen", "sonst_MLP"])
axes[1].set_title("Urania")
axes[1].set_yticks([])
...
urania_df = df[df['medium'] == 'urania']
aglaja_df = df[df['medium'] == 'aglaja']
