...
for filename in os.listdir(global_corpus_representation_directory(system)):
    if filename ==  "all_complex_features_matrix.csv":
        ...
        dtm_obj.data_matrix_df = years_to_periods(input_df=dtm_obj.data_matrix_df, category_name="Jahr_ED",
                                                  start_year=1750, end_year=1900, epoch_length=100,
        ...
        df_all = df.copy()
        df = df_all.loc[:, ["fraction_indirspeech","rom_setting","fear", "centralization","fraction_dirspeech",  "density", "Gattungslabel_ED_normalisiert", "periods", "Jahr_ED", "Nachname"]]
        ...
        for period in periods:
            ...
            df_1 = period_data[period_data[genre_cat] == "N"]

            for i in range(n):
                train_set, test_set = principled_sampling(df_1, df_0, select_one_per_period=False, select_one_per_author=False)

                indexes = period_data.index.tolist()
                features = period_data.columns.tolist()
            ...
            results_dict[period] = [np.mean(np.asarray(acc_scores)),np.mean(np.asarray(N_f1_scores)), np.mean(np.asarray(E_f1_scores))]
