...
for filename in os.listdir(global_corpus_representation_directory(system)):
    if filename ==  "all_complex_features_matrix.csv":
        ...
        for i in range(n):
            ...
            df_all = df.copy()
            df = df.loc[:,
                 ["fraction_dirspeech", "fraction_indirspeech", "rom_setting", "danger", "fear", "centralization",
                  "length", "Gattungslabel_ED_normalisiert", "Jahr_ED", "Nachname"]]
...
print(df)
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_features_coefs_N-E_after1850_complexfeatures.csv"))
...
plt.savefig("/home/julian/git/PyNovellaHistory/figures/gridsuche_c-at-one_N-E_complex_features_after1850.svg")
plt.show()


outfile_output = os.path.join(local_temp_directory(system), "output_av_pred_probabs_N-E_after1850_complexfeatures.txt")
...
df.to_csv(path_or_buf=os.path.join(local_temp_directory(system), "av_pred_probabs_N-E_after1850_complexfeatures.csv"))
