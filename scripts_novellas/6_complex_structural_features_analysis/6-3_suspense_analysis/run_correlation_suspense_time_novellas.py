...
from scipy.stats import pearsonr, spearmanr, siegelslopes, levene
...
novellas_df = novellas_df.rename(columns={"Titel": "title", "Nachname":"author", "Jahr_ED":"date",
                                          ...
                                          "max_value": "Gefahrenlevel", "lin_susp_model": "Spannungsindikator"})

#whole_df = pd.concat([heftromane_df,novellas_df])
whole_df = novellas_df
...
whole_df = whole_df.drop(whole_df['Gefahrenlevel'].idxmax())
...
whole_df["Spannungsindikator"] = whole_df.apply(lambda x: x.Gefahrenlevel + (0.725 * x.Angstempfinden), axis=1)

scaler = MinMaxScaler()

scaled_values = scaler.fit_transform(whole_df[["Gewaltverbrechen", "Kampf", "Entführung", "Krieg", "Spuk","Gefahrenlevel", "Angstempfinden", "Sturm", "Feuer", "Spannungsindikator"]])
whole_df[["Gewaltverbrechen", "Kampf", "Entführung", "Krieg","Spuk","Gefahrenlevel", "Angstempfinden", "Sturm", "Feuer", "Spannungsindikator"]] = scaled_values
...
danger_df["Gefahrenlevel"] = danger_df[["Gewaltverbrechen", "Kampf", "Entführung","Krieg","Sturm","Feuer"]].max(axis=1)
...
y_variable = "Spannungsindikator"
x_variables = [year_cat_name]

if y_variable == "Gefahrenlevel":
...
sns.lineplot(data=df, x="date", y="Spannungsindikator", hue="seriell")
...
whole_df.boxplot(column="Spannungsindikator", by="seriell")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

df.boxplot(column="Spannungsindikator", by="medium")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

whole_df.boxplot(column="Gefahrenlevel", by="medium")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

whole_df.boxplot(column="Gefahrenlevel", by="genre")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()
...
whole_df.boxplot(column="Gefahrenlevel", by=["periods", "genre"])
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

whole_df.boxplot(column="Gefahrenlevel", by=["genre", "periods"])
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()


whole_df.boxplot(column="Gefahrenlevel", by="periods")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

whole_df.boxplot(column="Angstempfinden", by="periods")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

whole_df.boxplot(column="Spannungsindikator", by="periods")
plt.xticks(rotation=90)
plt.tight_layout()
plt.show()

df1800 = whole_df[whole_df["periods"] == "1750-1850"]
df1850 = whole_df[whole_df["periods"] == "1850-1950"]
stat, p = levene(df1800.Gefahrenlevel, df1850.Gefahrenlevel)
print("Levene test 1800s vs 1850s for al texts for danger level: ", stat, p)

stat, p = levene(df1800.Gefahrenlevel, df1850.Angstempfinden)
print("Levene test 1800s vs 1850s for al texts for Angstempfinden: ", stat, p)

df1800_N = df1800[df1800["genre"] == "Novelle"]
df1850_N = df1850[df1850["genre"] == "Novelle"]
stat, p = levene(df1800_N.Gefahrenlevel, df1850_N.Gefahrenlevel)
print("Levene test 1800s vs 1850s for Novellen: ", stat, p)


print("around 1800 proportion of texts with danger level 0: ", df1800[df1800["Gefahrenlevel"] == 0].shape[0] / df1800.shape[0])
print("around 1900 proportion of texts with danger level 0: ", df1850[df1850["Gefahrenlevel"] == 0].shape[0] / df1850.shape[0])

print("around 1800 proportion of Novellen with danger level 0: ", df1800_N[df1800_N["Gefahrenlevel"] == 0].shape[0] / df1800_N.shape[0])
print("around 1900 proportion of Novellen with danger level 0: ", df1850_N[df1850_N["Gefahrenlevel"] == 0].shape[0] / df1850_N.shape[0])
