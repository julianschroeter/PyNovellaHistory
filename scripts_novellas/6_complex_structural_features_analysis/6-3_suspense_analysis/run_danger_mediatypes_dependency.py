...
from preprocessing.metadata_transformation import standardize_meta_data_medium
import os
...
df_media = standardize_meta_data_medium(df=df, medium_column_name=media_cat_name)

print(df_media)


y_variable = "Angstempfinden"
x_variables = ["Kampf", "Krieg", "Gewaltverbrechen", "Erotik", "Sturm", "Feuer","max_value"]
...
max_matrix = max_matrix.add_metadata(media_cat_name)
df = max_matrix.data_matrix_df
df[media_cat_name] = df[media_cat_name].fillna("unknown")
df = standardize_meta_data_medium(df=df, medium_column_name=media_cat_name)

genres_list = ["N", "E", "R", "0E", "XE", "M"]
media_list = list(set(df["medium"].values))

media_list = ['daheim', 'grimm_khm', 'pantheon', 'urania', 'dtrundsch', 'aglaja', 'gartenlaube']

print(media_list)
df = df[df.isin({"medium": media_list}).any(1)]

print(df)

df.boxplot(column="Kampf", by="medium")
#plt.title("Boxplots: Dominanz von Kampf-Vokabular nach Medienformaten")
plt.show()

df.boxplot(column="Krieg", by="medium")
plt.show()

df.boxplot(column="Erotik", by="medium")
plt.show()

df.boxplot(column="max_value", by="medium")
plt.show()

df.boxplot(column="Gewaltverbrechen", by="medium")
plt.show()

df.boxplot(column="Sturm", by="medium")
plt.show()

df.boxplot(column="Feuer", by="medium")
plt.show()

colors_list = ["red", "green", "blue", "cyan", "brown","yellow", "pink"]
zipped_dict = dict(zip(media_list, colors_list[:len(colors_list)]))

mpatches_list = []
for media, color in zipped_dict.items():
    patch = mpatches.Patch(color=color, label=media.capitalize())
...
#    fig, ax = plt.subplots()
 #   clf = NearestCentroid()
  #  clf.fit(df[[x_variable, y_variable]].values, df["medium"])
...
#
 #   new_colors_list = [zipped_dict[k] for k in class_labels if k in colors_list]
  #  ax.scatter(centroids[:, 0], centroids[:, 1], color=new_colors_list)
   ...
   #                                               [centroids[class_labels.index(x["medium"])][0],
    #                                               centroids[class_labels.index(x["medium"])][1]]), axis=1)
...
#
 #       if class_label in genres_list:
  #          print(class_label)
   #         class_df = df[df["medium"] == class_label]
#
 ...
 #           plt.annotate(text=str(class_labels[i]), xy=xy, color=color)
  #  plt.show()
    ...
    for media, color in zipped_dict.items():
        ...
        genre_obj = genre_obj.reduce_to_categories("medium", [media])
        genre_obj = genre_obj.eliminate([media_cat_name])
        genre_df = genre_obj.data_matrix_df
        print(genre_df)
        if color == "red":
            plt.scatter(genre_df.loc[:, x_variable], genre_df.loc[:, y_variable], marker= "o", color=color, label=media)
        else:
            plt.scatter(genre_df.loc[:,x_variable], genre_df.loc[:, y_variable], marker = "o", color=color, label=media, alpha=1)
