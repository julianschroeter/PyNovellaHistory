...
outfile_csv_path = str(n) + "_" + metric + "_distances_media-formats_" + filename
outfile_dict_txt_path = str(n) + "_" + metric + "_distances-media-formats_" + filename_base + ".txt"
...
df_TB = genre_dtm_obj.reduce_to_categories(metadata_category=medium_cat, label_list=["Taschenbuch"]).data_matrix_df
#df_N_TB.loc[:,genre_cat] = "Taschenbuch-Novelle"
df_Anth = genre_dtm_obj.reduce_to_categories(metadata_category=medium_cat, label_list=["Anthologie"]).data_matrix_df
#df_N_Anth.loc[:,genre_cat] = "Anthologie-Novelle"
df_Fambl = genre_dtm_obj.reduce_to_categories(metadata_category=medium_cat, label_list=["Familienblatt"]).data_matrix_df
#df_N_FamBl.loc[:,genre_cat] = "Familienblatt-Novelle"
df_Rundsch = genre_dtm_obj.reduce_to_categories(metadata_category=medium_cat, label_list=["Rundschau"]).data_matrix_df
#df_N_Rundsch.loc[:,genre_cat] = "Rundschau-Novelle"

list_of_genre_dfs.append(["Taschenbuch", df_TB])
list_of_genre_dfs.append(["Anthologie", df_Anth])
list_of_genre_dfs.append(["Familienblatt", df_Fambl])
list_of_genre_dfs.append(["Rundschau", df_Rundsch])
...
all_df = all_df.drop(columns=[genre_cat])
...
for name_df_comb in genre_df_comb:
    ...
    if genre_cat in df_1.columns:
        df_1 = df_1.drop(columns=[genre_cat])
    df_2 = name_df_comb[1][1]
    if genre_cat in df_2.columns:
        df_2 = df_2.drop(columns=[genre_cat])
