...
dtm_obj.data_matrix_df = years_to_periods(dtm_obj.data_matrix_df,category_name=year_cat, start_year=1790, end_year=1930, epoch_length=20,
                                          new_periods_column_name="periods")
...
all_df = dtm_obj_red.data_matrix_df
print(all_df)

#all_df = df.drop(columns="periods")
#print(all_df)
...
outfile_df_name = str(n) + "_E_" + metric + "periods-distance_results_2500mfw.csv"
outfile_dict_name = str(n) + "_E_" + metric + "periods-min_max_distances_dict_2500mfw.txt"
...
dist_results_obj = DistResults(n, input_df_1=all_df, input_df_2=None, metric=metric, label1="all", label2=None, select_one_author=False, select_one_per_period=False)

for label, df in list_of_periods_dfs:
    dist_results_obj.add_result(n, input_df_1=df, label1=label, metric=metric, select_one_author=False, select_one_per_period=False)
