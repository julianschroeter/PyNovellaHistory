...
for i in range(20):
    ...
    for pair in combinations(list_of_periods_dfs,2):
        ...
        if abs(start_b - start_a) >= 40:
            ...
            print(metatdata_df.loc[distances["instance_of_df1_with_max_dist_to_df2_group"][0][0],["Nachname", "Titel", "Jahr_ED", "Gattungslabel_ED_normalisiert"]])
            print(distances["instance_of_df1_with_max_dist_to_df2_group"][0][1][1])
            list_of_nearest_precursors.append(distances["instance_of_df1_with_max_dist_to_df2_group"][0][0])

print(list_of_nearest_precursors)
nearest_precursors = Counter(list_of_nearest_precursors)
...
new_df.to_csv(os.path.join(local_temp_directory(system), "farest_followers.csv"))